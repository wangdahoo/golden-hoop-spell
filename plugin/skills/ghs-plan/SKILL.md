...
| Review stats | **Review file** (preferred): the `REVIEW COMPLETE \| Verdict: ... \| Severe: X Medium: Y Optimization: Z` signal line at the end of the report (always present on a well-formed review — prefer this), or an issue-list section (`## Issue Summary` / `## 问题清单` or equivalent) if the signal line is somehow unavailable. Fallback: the parser JSON `completion_signal` field (only if parsing succeeded and it is still in memory). | Extract from the review file. Do NOT infer counts from dispatcher memory or the JSON `verdict` field — `verdict` only holds PASS/FAIL/null. |
